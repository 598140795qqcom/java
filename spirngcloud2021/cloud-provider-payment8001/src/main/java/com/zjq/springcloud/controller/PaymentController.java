...
public class PaymentController {
    ...
    public CommonResult<Payment> update( Payment payment){
        ...
        log.info("插入结果"+result);
        if(result>0){
            return new CommonResult(200,"修改数据成功:serverPort"+serverPort,result);
        }else{
            return new CommonResult(500,"修改数据失败:serverPort"+serverPort,null);
        }
    }
    ...
    public CommonResult<Payment> getPaymentById(@PathVariable("id") long id) {
        ...
        log.info("11111111111111查1111111111111询结果11111111111" + result);
        if (result != null) {
            return new CommonResult(200, "查询数据成功111111111:serverPort"+serverPort, result);
        } else {
